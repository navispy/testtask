...
public class EditHumanController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        human = HumanHolder.getInstance().getHuman();
        String name = human.getName();
        int age = human.getAge();
        Date birthday = (java.sql.Date) human.getBirthday();
        LocalDateConverter conv = new LocalDateConverter();
        LocalDate local = conv.convertToEntityAttribute(birthday);

        tfName.setText(name);
        tfAge.setText(Integer.toString(age));
        dpBirthday.setValue(local);
